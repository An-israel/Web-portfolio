...
export default function HirePage() {
  ...
  async function submit() {
    ...
    try {
      ...
      // Cast: the generated DB types still describe the legacy inquiries
      // schema; Phase 6 recreates this table to match `payload`.
      const { error } = await supabase.from('inquiries').insert(payload as never);
      if (error) throw error;
      setDone(true);
    } catch {
      // Backend not wired yet (or transient failure): fall back to email so
      // no inquiry is ever lost, and still confirm to the visitor.
      try {
        const body = [
  ...
  const summary = useMemo(
    () => [
      ...
      {
        label: 'Budget',
        value: isSalaryRole ? 'Salary role' : form.budget_range,
        step: 2,
      },
          ...
          {step === 3 && (
            <div className="space-y-5">
              ...
              <Field
                ...
                onChange={(v) => set('role_at_company', v)}
                placeholder="Optional"
              />
              <div>
