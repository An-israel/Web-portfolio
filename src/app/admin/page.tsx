...
async function getData() {
  try {
    ...
    const [newInq, monthInq, views7, publishedProjects, latest, newBriefs, pendingEnrol] = await Promise.all([
      ...
      supabase.from('client_briefs').select('*', { count: 'exact', head: true }).eq('status', 'submitted'),
    ...
    return {
      ...
      pendingEnrol: pendingEnrol.count || 0,
...
export default async function AdminOverview() {
  ...
  const stats = [
    ...
    { label: 'NEW CLIENT BRIEFS', value: d.newBriefs, href: '/admin/briefs', hot: d.newBriefs > 0 },
    { label: 'ENROLMENTS AWAITING PAYMENT', value: d.pendingEnrol, href: '/admin/enrolments', hot: d.pendingEnrol > 0 },
      ...
      <div className="mt-10 rounded-md border border-[var(--steel)] bg-[var(--graphite)]">
        ...
        {d.latest.length === 0 ? (
          <p className="px-5 py-10 text-center text-sm text-[var(--mist)]">
            No inquiries yet — share your /hire or /services link.
          </p>
        ) : (
